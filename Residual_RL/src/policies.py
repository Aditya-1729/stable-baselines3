...
class Guide_policy:
    ...
    def predict(self):
        ...
        self.action[12:14] = eef_pos[:2] + np.clip(self.env.site_pos[:2]-eef_pos[:2], a_min=np.array([-self.position_limits, -self.position_limits]), a_max=np.array([self.position_limits, self.position_limits]))
        self.action[-1] = eef_pos[-1] + np.clip(self.env.site_pos[-1] - self.indent - eef_pos[-1], a_min = -self.position_limits, a_max=self.position_limits)        
...
class ResidualSAC(SAC):
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            if self.num_timesteps > self.burn_in_steps: #_total_timesteps
        ...
        self.logger.record("train/ent_value", np.mean(ent_value)) # added to log entropy
    ...
    def predict(self,
        ...
        #     print(self.policy.actor.action_dist.distribution.mean[:3])  

        # action, state = HybridPolicy2(action,state,self.env).predict()
        # print(action)
...
class HybridPolicy:
    ...
    def predict (self):
        ...
        # print(f'agent_action:{self.action}')
        guide_action = Guide_policy(self.env).predict()[True]
        # print(f'guide_action:{guide_action}')
        self.final_action = guide_action + self.action
        # print(f'final_action:{self.final_action}')
        return self.final_action, self.state  
        # return guide_action, self.state
...
class HybridPolicy2:
    ...
    def predict (self):
        ...
        self.final_action = guide_action + self.action
        self.final_action[0][-3:] = eef_pos + np.clip(guide_action[0][-3:] + self.action[0][-3:], a_min=np.ones(3) * (-self.position_limits),\
