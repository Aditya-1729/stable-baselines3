...
class PerformanceLog(EvalCallback):
    """
    This class logs the performance of the model after every evaluation.
    
    It includes the mean reward, mean episode length, mean force, mean deviation, via points wiped and table height.
    
    The condition for saving model is when its previous best reward is superseded and to ground the model with physical performance
    the mean force should be between 4.85 and 5.15, the number of via points wiped should be 8.
    """
...
class Training_info(BaseCallback):
    """
    This class logs the safety triggers during training, which include:
    the instances of collisions, excessive force and the robot reaching its joint limits.
    """
