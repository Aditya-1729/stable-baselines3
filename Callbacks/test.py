...
class BestWandb(EvalCallback):
    def __init__(self, use_wandb=True, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.log_idx=self.training_env.envs[0].total_steps
    def _on_step(self):
...
def eval_policy(
            ...
            if episode_counts[i] < episode_count_targets[i]:
                ...
                if use_wandb:
                    metrics = { 'Wipe/reward': reward,
                                ...
                                "Wipe/reward_done": self.reward_done,
                                "Wipe/reward_contact": self.reward_contact,
                                "Wipe/penalty_yvel": self.penalty_yvel,
                                "Wipe/penalty_xdist": self.penalty_xdist,
                                "Wipe/penalty_force": self.penalty_force,
                                "Wipe/penalty_xvel": self.penalty_xvel,
                                    ...
                                    'Wipe/vel_x': env.envs[0].robots[0]._hand_vel[0],
                                    # 'Wipe/del_z': delta[2],
    ...
    return episode_rewards, episode_lengths, episode_force, episode_deviation, episode_via_points, table_height
...
class PerformanceLog(EvalCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            episode_rewards, episode_lengths, episode_forces,episode_deviations, episode_wiped, table_height= eval_policy(
                self.use_wandb,
                self.cfg,
                self.model,
                self.eval_env,
            ...
            mean_ep_force, std_ep_force, max_ep_force, min_ep_force = np.nanmean(episode_forces), np.nanstd(episode_forces) , np.nanmax(episode_forces), np.nanmin(episode_forces)
            ...
            self.logger.record("eval/table_height", table_height)
    ...
    while (episode_counts < episode_count_targets).any():
        ...
        for i in range(n_envs):
            if episode_counts[i] < episode_count_targets[i]:
                ...
                if callback is not None:
                    callback(locals(), globals())
                metrics = { f'Wipe_{log_idx}/reward': reward,
                                f'Wipe_{log_idx}/Return': Return,
                                # f'Wipe_{log_idx}/force': info['force'],
                                # f'Wipe_{log_idx}/unadjusted_force': env.envs[0].robots[0].ee_force,
                                # f"Wipe_{log_idx}/task_complete_reward": env.envs[0].task_completion_r,
                                # f"Wipe_{log_idx}/excess_force_penalty": env.envs[0].force_penalty,
                                # f"Wipe_{log_idx}/low_force_penalty":env.envs[0].low_force_penalty,
                                # f"Wipe_{log_idx}/distance_penalty":-env.envs[0].total_dist_reward,
                                # f"Wipe_{log_idx}/wipe_contact_reward": env.envs[0].wipe_contact_r,
                                # f"Wipe_{log_idx}/unnormalized_reward": env.envs[0].un_normalized_reward,
                                # f"Wipe_{log_idx}/unit_wiped_reward": env.envs[0].unit_wipe,
                                # # f"Wipe_{log_idx}/time_ratio":t_contact/t,
                                # f"Wipe_{log_idx}/force_in_window_penalty":env.envs[0].force_in_window_penalty,
                                # f'Wipe_{log_idx}/ncontacts': env.envs[0].sim.data.ncon,
                                # f'Wipe_{log_idx}/del_x': info['deviation'],
                                # f'Wipe_{log_idx}/vel_y': env.envs[0].robots[0]._hand_vel[1],
                                # # f'Wipe_{log_idx}/del_z': delta[2],
                                # f'Wipe_{log_idx}/reward_wop': env.envs[0].reward_wop,
                                }
